...
from config import load_text_update_groups

app = Flask(__name__)
twilio = TwilioClass.TwilioClient()
nocodb = NocoClass.NocoClass()
text_keywords = load_text_update_groups()
...
def sms_reply():
    ...
    elif body in text_keywords.text_update_groups:
        for keyword, group_data in text_keywords.text_update_groups.items():
            if keyword == body:
                sender_phone_number = request.values.get("From", None)
                subscriber_status = nocodb.check_if_subscribed(
                    subscriber_number=sender_phone_number,
                    group_to_check=group_data.nocodb_tag,
                )
                sender_phone_number.replace("+", "")
                int(sender_phone_number)
                if subscriber_status:
                    ...
                    nocodb.add_subscriber(
                        sender_phone_number, group=group_data.nocodb_tag
                    )
                    resp.message(
...
if __name__ == "__main__":
    print(text_keywords)
    print(text_keywords.text_update_groups)
    # print(text_keywords.text_update_groups.items())
